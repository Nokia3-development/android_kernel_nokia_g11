...
struct sprd_i2c {
	...
	int irq;
	int err;
	bool ack_flag;//Modified by xuzhanzhi for bringup nfc
};
...
static irqreturn_t sprd_i2c_isr_thread(int irq, void *dev_id)
{
	...
	struct i2c_msg *msg = i2c_dev->msg;
	//bool ack = !(readl(i2c_dev->base + I2C_STATUS) & I2C_RX_ACK);//Modified by xuzhanzhi for bringup nfc
	u32 i2c_tran;
	 ...
	 */
	if (i2c_tran &&  i2c_dev->ack_flag) {//Modified by xuzhanzhi for bringup nfc
		sprd_i2c_data_transfer(i2c_dev);
		return IRQ_HANDLED;
	}
	 ...
	 */
	if (!i2c_dev->ack_flag)//Modified by xuzhanzhi for bringup nfc
		i2c_dev->err = -EIO;
...
static irqreturn_t sprd_i2c_isr(int irq, void *dev_id)
{
	 ...
	 * transmission too.
	 */
	i2c_dev->ack_flag = !(readl(i2c_dev->base + I2C_STATUS) & I2C_RX_ACK);//Modified by xuzhanzhi for bringup nfc
