		...
		struct drm_display_mode *mode)
{
	...
	if (vrr_mode) {
		...
		//panel->info.mode.vsync_end = mode->vsync_end;
		//panel->info.mode.vtotal = mode->vtotal;
	} else {
		if ((mode->hdisplay != ctx->vm.hactive) ||
			(mode->vdisplay != ctx->vm.vactive))
			enhance->need_scale = true;
...
{
/*Modified by gonglinmin@sagereal.com for MGK-1503 Screen flash Begin 2022-03-25*/
#ifdef TARGET_PRODUCT_MGK
	cap->max_layers = 1;
#else
	cap->max_layers = 4;
#endif
/*Modified by gonglinmin@sagereal.com for MGK-1503 Screen flash End 2022-03-25*/
