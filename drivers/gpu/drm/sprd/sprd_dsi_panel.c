...
/* Added by qinjinke@sagereal.com for MGK-621 Touchpanel Gesture Begin 2022-04-13 */
/* Added by zeng_ji@hoperun.com for SCP-5013 Touchpanel Gesture Begin 2022-06-16 */
#if defined(TARGET_PRODUCT_MGK) || defined(TARGET_PRODUCT_SCP) || defined(TARGET_PRODUCT_SCT)
#include <linux/proc_fs.h>  	
#include <asm/uaccess.h>
#endif
/* Added by zeng_ji@hoperun.com for SCP-5013 Touchpanel Gesture End 2022-06-16 */
/* Added by qinjinke@sagereal.com for MGK-621 Touchpanel Gesture End 2022-04-13 */
...
u8 cmd_password0[3] = {0xF0,0x5A,0x59};
u8 cmd_password1[3] = {0xF1,0xA5,0xA6};
u8 cmd_vref[2] = {0xF6,0x00};
u8 cmd_read[1] = {0x00};

unsigned int sprd_strtoint_vref(char * str,int len)
{
	unsigned int ret_vref = 0;
	while(len-- > 0) {
		ret_vref <<= 4;
		if((*str >= '0' && *str <= '9')) {
			ret_vref |= *str - '0';
	...
	DRM_INFO("sprd_strtoint_vref : %d",ret_vref);
	return ret_vref;
}
#endif
...
/*Added by qinjinke@sagereal.com for MGK-961 icnl9911c VREF End 2022-04-28*/
	...
	while (size > 0) {
		...
		/*Added by qinjinke@sagereal.com for MGK-961 icnl9911c VREF Begin 2022-04-28*/
#ifdef TARGET_PRODUCT_MGK
		if(cmds->payload[0] == 0xF6) {
			if(strncmp(panel->lcd_name,"lcd_icnl9911c_hx_mdt_mipi_hdp",strlen("lcd_icnl9911c_hx_mdt_mipi_hdp")) == 0) {
				mipi_dsi_dcs_write_buffer(dsi, cmd_password0, sizeof(cmd_password0));
				mipi_dsi_dcs_write_buffer(dsi, cmd_password1, sizeof(cmd_password1));
				regif = sprd_strtoint_vref(panel->vref_reg,2);
				cmd_vref[1] = regif;
				mipi_dsi_dcs_write_buffer(dsi, cmd_vref, sizeof(cmd_vref));
				mipi_dsi_dcs_read(dsi, 0xF6,cmd_read, sizeof(cmd_read));
				DRM_INFO("%s() :vref_read_cmd1 = 0x%x\n", __func__,cmd_read[0]);
			}
		}
#endif
		/*Added by qinjinke@sagereal.com for MGK-961 icnl9911c VREF End 2022-04-28*/
...
/* Added by zeng_ji@hoperun.com for SCP-5013 Touchpanel Gesture Begin 2022-06-16 */
#if defined(TARGET_PRODUCT_MGK) || defined(TARGET_PRODUCT_SCP) || defined(TARGET_PRODUCT_SCT)
static uint32_t panel_gesture = 0;
static struct proc_dir_entry *panel_gesture_proc = NULL;
static ssize_t panel_gesture_read(struct file *file, char __user *page, size_t size, loff_t *ppos)
{
	char buf[50]={0};
	int count = 0;
	if (*ppos)
	...
	count = sprintf(buf, "panel_gesture is %d\n", panel_gesture);
...
static ssize_t panel_gesture_write(struct file *filp, const char __user *buf, size_t count, loff_t *off)
{
	char inbuffer[4]={0};
	if(count>4) {
	    count=4;
	...
	if (sscanf(inbuffer, "%d",  &panel_gesture) == 1) 
	{
		DRM_INFO("panel_gesture= %d\n",panel_gesture);
	}
	if(panel_gesture==1) {
		DRM_INFO("panel_gesture= %d\n",panel_gesture);
	} else {
...
static const struct file_operations config_proc_ops = {
    .owner = THIS_MODULE,
    .read = panel_gesture_read,
    .write = panel_gesture_write,	
};
#endif
/* Added by zeng_ji@hoperun.com for SCP-5013 Touchpanel Gesture End 2022-06-16 */
/* Added by qinjinke@sagereal.com for MGK-621 Touchpanel Gesture End 2022-04-13 */
bool g_gesture_flag = 0;	//Modify for SCP-5013,lcd use for tp gesture
//EXPORT_SYMBOL_GPL(g_gesture_flag);	//Modify for SCP-5013,lcd use for tp gesture
...
/* Added by qinjinke@sagereal.com for MGK-621 Touchpanel Gesture End 2022-04-13 */
	...
	// Modify for SCP-5013,lcd use for tp gesture
	if(g_gesture_flag == 0) {
...
#ifdef TARGET_PRODUCT_SCP
	DRM_INFO("%s(), panel->lcd_name=%s\n", __func__, panel->lcd_name);
	if((!strcmp(panel->lcd_name, "lcd_icnl9911_skyworth_mipi_hdp"))||(!strcmp(panel->lcd_name, "lcd_icnl9911_skyworth_mantix_mipi_hdp"))) {
		mipi_dsi_dcs_write_buffer(panel->slave, cmd_password0, sizeof(cmd_password0));
		mipi_dsi_dcs_write_buffer(panel->slave, cmd_password1, sizeof(cmd_password1));
		regif = sprd_strtoint_vref(panel->vref_reg, 2);
		cmd_vref[1] = regif;
		mipi_dsi_dcs_write_buffer(panel->slave, cmd_vref, sizeof(cmd_vref));
		mipi_dsi_dcs_read(panel->slave, 0xF6, cmd_read, sizeof(cmd_read));
		DRM_INFO("%s():regif= 0x%x, vref_read_cmd1 = 0x%x\n", __func__, regif, cmd_read[0]);
	}
#endif
...
static int sprd_panel_parse_dt(struct device_node *np, struct sprd_panel *panel)
{
	...
	/*Added by qinjinke@sagereal.com for MGK-961 icnl9911c VREF Begin 2022-04-28*/
#if defined(TARGET_PRODUCT_MGK) || defined(TARGET_PRODUCT_SCP)
	const char *cmd_line, *lcd_name_p, *vref_reg_p;
#else
	...
	if (!rc) {
		...
		/*Added by qinjinke@sagereal.com for MGK-961 icnl9911c VREF Begin 2022-04-28*/
#if defined(TARGET_PRODUCT_MGK) || defined(TARGET_PRODUCT_SCP)
		vref_reg_p = strstr(cmd_line, "vref_reg=");
#endif
		/*Added by qinjinke@sagereal.com for MGK-961 icnl9911c VREF End 2022-04-28*/
		if (lcd_name_p) {
...
#if defined(TARGET_PRODUCT_MGK) || defined(TARGET_PRODUCT_SCP)
		if (vref_reg_p) {
			sscanf(vref_reg_p, "vref_reg=%s", panel->vref_reg);
			DRM_INFO("vref reg: %s\n", panel->vref_reg);
		}
#endif
		/*Added by qinjinke@sagereal.com for MGK-961 icnl9911c VREF End 2022-04-28*/
...
/* Added by zeng_ji@hoperun.com for SCP-5013 Touchpanel Gesture Begin 2022-06-16 */
#if defined(TARGET_PRODUCT_MGK) || defined(TARGET_PRODUCT_SCP) || defined(TARGET_PRODUCT_SCT)
	panel_gesture_proc = proc_create("panel_gesture", 0666, NULL, &config_proc_ops);
	if (panel_gesture_proc == NULL) {
		DRM_INFO("create_proc_entry %s failed\n", "panel_gesture");
	} else {
		DRM_INFO("create_proc_entry %s success\n", "panel_gesture");
	}
#endif	
/* Added by zeng_ji@hoperun.com for SCP-5013 Touchpanel Gesture End 2022-06-16 */
/* Added by qinjinke@sagereal.com for MGK-621 Touchpanel Gesture End 2022-04-13 */
